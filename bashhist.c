...
static void log_history_remotely PARAMS((char *out));
...
void log_history_remotely(char *out) {
  /* code */
  ...
  if (c != NULL && c->err) {
    ...
    if(sudo_user!=NULL){
      reply = redisCommand(c, "HMSET %s:%s:%s:%d %dl %s",host_name,sudo_user,user_name,pid,ts,out);
    }
    else{
      reply = redisCommand(c, "HMSET %s:%s:%d %dl %s",host_name,user_name,pid,ts,out);
    }
  }
  redisFree(c);
}
